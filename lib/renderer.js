...
class GravissRenderer {
  ...
  createBeamGeometry(nodesById, elements) {
    ...
    // Strict like the shell fill, and unbiased against it, so the boundary
    // between grazing bodies sits where the geometry puts it at every zoom.
    // The member's own contours cannot rely on ties — they are transformed on
    // the CPU and the fill on the GPU, which round differently — so their
    // profile is inflated a hair instead, in createMemberContours.
    ...
    for (const group of groups.values()) {
      ...
      // lose depth ties, so a fill stays behind its own lines while a fill in
      // front of a foreign line by any distance paints over it, exactly to
      // the intersection.
  ...
  createMemberContours(memberMesh) {
    ...
    // A hair outside the section rather than on it: these are stamped on the
    // CPU while the fill transforms on the GPU, and the two round differently,
    // so an arris left exactly on the surface would win or lose its depth
    // ties pixel by pixel. A tenth of a percent of the profile stands it
    // clear of the fill and under any distance a picture shows.
    unitEdges.scale(1, 1.001, 1.001);
  ...
  createShellGeometry(nodesById, elements) {
    ...
    // The fill tier, after every line: losing depth ties it stays behind its
    // own edge lines, and in front of a foreign line — a member's arris
