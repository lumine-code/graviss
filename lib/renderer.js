...
class GravissRenderer {
  ...
  updateShellSurface() {
    ...
    const shared = sharedNodeSurfaces(state.elements, this.nodesById);
    state.shared = shared;
    state.elements.forEach((element, index) => {
      const item = state.prepared[index];
      item.normals = cornerDisplacementNormals(element, item.nodes, shared.normals);
    });
  ...
  createShellGeometry(nodesById, elements) {
    ...
    // deciding its topology a second time.
    this.shellState = { prepared, shared, positions, edgePositions, elements };
