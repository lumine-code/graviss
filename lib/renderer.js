...
class GravissRenderer {
  ...
  applyBackground(appearance) {
    ...
    for (let index = 0; index < position.count; index += 1) {
      ...
      // Nadir to zenith, measured against the model's declared up axis rather
      // than the world's, so a Z-down model grades the way it stands.
      const height = direction.dot(this.worldUp) * 0.5 + 0.5;
      color
        .copy(base)
        .lerp(lifted, height)
