...
class GravissRenderer {
  ...
  createBeamGeometry(nodesById, elements) {
    ...
    // The same polygon offset as the shell fill, though no line is drawn on a
    // member's own faces that needs it: the offset sinks a fill by a depth
    // quantum, whose world size follows the zoom, so a fill left at true depth
    // would meet an offset neighbour a quantum early — and where a beam grazes
    // the slab it runs through, that boundary slides along the surface as the
    // zoom changes. Sunk equally, the bias cancels and the intersection stays
    // where it is.
