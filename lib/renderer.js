...
class GravissRenderer {
  ...
  createShellGeometry(nodesById, elements) {
    ...
    for (const element of elements) {
      ...
      if (layers === 2) edgeCount += element.nodeIds.length;
    }
    ...
    const entityRanges = [];
    const shared = sharedHalfThickness(elements);
    ...
    elements.forEach((element, entityIndex) => {
      ...
      const half = layers === 2 ? elementHalfThickness(element, nodes.length, shared) : null;
      ...
      const middle = elementOffsets(element, nodes.length);
      ...
      if (layers === 2) {
        ...
        for (let nodeIndex = 0; nodeIndex < nodes.length; nodeIndex += 1) {
          ...
          const corners = [
            [startNode, middle[nodeIndex] + half[nodeIndex]],
            [endNode, middle[nextIndex] + half[nextIndex]],
            [endNode, middle[nextIndex] - half[nextIndex]],
            [startNode, middle[nodeIndex] - half[nodeIndex]],
          ];
      ...
      for (let layer = 0; layer < layers; layer += 1) {
        ...
        for (let nodeIndex = 0; nodeIndex < nodes.length; nodeIndex += 1) {
          ...
          edgePositions[edgeCursor * 6] = startNode.x + faceNormal.x * from;
          edgePositions[edgeCursor * 6 + 1] = startNode.y + faceNormal.y * from;
          edgePositions[edgeCursor * 6 + 2] = startNode.z + faceNormal.z * from;
          edgePositions[edgeCursor * 6 + 3] = endNode.x + faceNormal.x * to;
          edgePositions[edgeCursor * 6 + 4] = endNode.y + faceNormal.y * to;
          edgePositions[edgeCursor * 6 + 5] = endNode.z + faceNormal.z * to;
          edgeCursor += 1;
        }
      }
      if (layers === 2) {
        for (let nodeIndex = 0; nodeIndex < nodes.length; nodeIndex += 1) {
          ...
          edgePositions[edgeCursor * 6] = node.x + faceNormal.x * above;
          edgePositions[edgeCursor * 6 + 1] = node.y + faceNormal.y * above;
          edgePositions[edgeCursor * 6 + 2] = node.z + faceNormal.z * above;
          edgePositions[edgeCursor * 6 + 3] = node.x + faceNormal.x * below;
          edgePositions[edgeCursor * 6 + 4] = node.y + faceNormal.y * below;
          edgePositions[edgeCursor * 6 + 5] = node.z + faceNormal.z * below;
          edgeCursor += 1;
        }
      }
    });
  ...
  elementCenter(element, nodesById = this.nodesById, target = new this.THREE.Vector3()) {
    ...
    const offset = meanOffset(element);
...
// Where the element's surface sits relative to each of its nodes. One number is
// that distance everywhere; a list is one per node, which is what an eccentric
// element that tapers needs — its nodes sit on a face of the plate, and a face
// is a different distance from the middle wherever the plate is a different
// thickness. A single distance would hold the thin corners off their own nodes.
function elementOffsets(element, count) {
  ...
  return new Array(count).fill(Number.isFinite(declared) ? declared : 0);
}
...
// The mean half-thickness of the elements meeting at each node, over the ones
// that state a single thickness. An element stating one per node has said what
// its corners measure and is left alone, and takes no part in the mean either.
function sharedHalfThickness(elements) {
  const totals = new Map();
  for (const element of elements) {
    const declared = element.thickness;
    if (Array.isArray(declared) || !(declared > 0)) continue;
    for (const nodeId of element.nodeIds) {
      const key = stableIdKey(nodeId);
      const entry = totals.get(key) || { sum: 0, count: 0 };
      entry.sum += Math.abs(declared) / 2;
      entry.count += 1;
      totals.set(key, entry);
    }
  }
  const shared = new Map();
  for (const [key, { sum, count }] of totals) shared.set(key, sum / count);
  return shared;
}
