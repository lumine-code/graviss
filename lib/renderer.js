    ...
    elements.forEach((element, entityIndex) => {
      ...
      for (let layer = 0; layer < layers; layer += 1) {
        ...
        for (const triangle of triangles) {
          for (const nodeIndex of triangle) {
            ...
            const side = middle + away * (half ? half[nodeIndex] : 0);
      ...
      if (layers === 2) {
        ...
        for (let nodeIndex = 0; nodeIndex < nodes.length; nodeIndex += 1) {
          ...
          const corners = [
            [startNode, middle + half[nodeIndex]],
            [endNode, middle + half[nextIndex]],
            [endNode, middle - half[nextIndex]],
            [startNode, middle - half[nodeIndex]],
          ];
      ...
      for (let layer = 0; layer < layers; layer += 1) {
        ...
        for (let nodeIndex = 0; nodeIndex < nodes.length; nodeIndex += 1) {
          ...
          const from = middle + away * (half ? half[nodeIndex] : 0);
          const to = middle + away * (half ? half[nextIndex] : 0);
...
// Half the thickness at each node of an area element. One number is that number
// everywhere; a list is one per node, which is how a slab that tapers across
// itself is described. A list shorter than the element repeats its last value,
// since a provider naming fewer corners than it has means the rest are the
// same.
