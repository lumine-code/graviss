...
class GravissRenderer {
  ...
  createBeamGeometry(nodesById, elements) {
    ...
    // The same hair of polygon offset as the shell fill, for both of its
    // jobs: losing the tie against the member's own contour lines, and
    // meeting the shell fill unbiased — sunk unequally, the boundary between
    // grazing bodies slides along the surface as the zoom changes the world
    ...
    for (const group of groups.values()) {
      ...
      // Every fill draws after every line. Lines write true depth and fills
      // are sunk by their offset, so a fill fails against its own lines and
      // stays behind them, while a fill in front of a foreign line paints
      // over the sliver its sunk depth would otherwise let pierce along a
  ...
  createShellGeometry(nodesById, elements) {
    ...
    // A hair of polygon offset, in depth quanta and no slope term. The lines
    // draw first and write true depth, so all a fill needs is to lose the tie
    // against its own lines — two quanta cover the arithmetic noise between
    // the two rasterizations. A slope-scaled factor is worth centimetres of
    // depth at an oblique view, and where two bodies graze, anything in front
    // by less than the offset loses to the other body's lines: that was the
    // needle of mesh lines piercing along a grazing intersection, growing and
    // shrinking with the zoom as the offset's world size did.
    ...
    // The fill tier, after every line: sunk by its offset it fails against
    // its own edge lines, and in front of a foreign line — a member's arris
    // grazing beneath a slab — it paints over what it hides.
