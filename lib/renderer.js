...
// How far a graded background is lifted towards white above the horizon and
// dropped towards black below it. Two grades from one colour: a lit ceiling
// over a floor in shadow, which is what a room looks like and what tells the
// eye which way is up before it has found anything in the model to say so.
// Enough to read as light and not so much that the model has to compete with
// the room behind it.
const BACKGROUND_GRADIENT_LIFT = 0.42;
const BACKGROUND_GRADIENT_DROP = 0.26;
...
class GravissRenderer {
  ...
  applyBackground(appearance) {
    ...
    const dropped = base.clone().lerp(new THREE.Color(0x000000), BACKGROUND_GRADIENT_DROP);
