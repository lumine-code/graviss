...
class GravissRenderer {
  ...
  createBeamGeometry(nodesById, elements) {
    ...
    for (const group of groups.values()) {
      ...
      const matrices = group.elements.map((element) => this.beamElementMatrix(element));
  ...
  beamElementMatrix(element) {
    const THREE = this.THREE;
    const { quaternion, basis, xAxis, yAxis, zAxis, midpoint, scale } = this.beamScratch();
  ...
  // A stored region is metres on the plane through the target, along the
  // camera's right and up axes; the viewport is fractions of whatever that
  // plane currently shows. These two are the whole bridge between them, and
  // both are free to leave the 0..1 square: a region larger than the pane is
  // a pane showing less of it.
  printRegionToViewportFractions(region) {
    const visible = this.visibleExtentAtTarget();
    if (!visible) return null;
    return {
      x: 0.5 + (region.right - region.width / 2) / visible.width,
      y: 0.5 - (region.up + region.height / 2) / visible.height,
  ...
  printRegionFromViewportFractions(fractions) {
    ...
    if (!visible) return null;
    return {
      right: (fractions.x + fractions.width / 2 - 0.5) * visible.width,
      up: (0.5 - fractions.y - fractions.height / 2) * visible.height,
      width: fractions.width * visible.width,
      height: fractions.height * visible.height,
    };
  }
  ...
  // What an export covers: the stored rectangle placed on the target plane, or
  // the whole model with a margin when no region was drawn. A stored region
  // resolves to the same metres from any pane and from no pane at all; the
  // camera decides what stands inside it.
    ...
    if (storedRegion) {
      ...
      return {
        center: this.controls.target
          .clone()
          .addScaledVector(basis.right, region.right)
          .addScaledVector(basis.up, region.up)
          .toArray(),
        width: region.width,
