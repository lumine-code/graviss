...
class GravissRenderer {
  ...
  createControls(camera, target = null) {
    // A pivot belongs to the gesture that pinned it, and every caller that
    // reaches here has moved the camera out from under it.
  ...
  pointerDepth(intersection, forward) {
    const alignment = this.raycaster.ray.direction.dot(forward);
  ...
  dollyTowardPointer(event) {
    ...
    const depth = this.pointerDepth(intersection, forward);
    if (!(depth > 0)) return false;
    const next = Math.min(
      this.controls.maxDistance,
      Math.max(this.controls.minDistance, depth * Math.pow(1 - DOLLY_DEPTH_FRACTION, notches)),
    );
    const travel = (depth - next) / this.raycaster.ray.direction.dot(forward);
    ...
    this.controls.target.copy(this.camera.position).addScaledVector(forward, next);
