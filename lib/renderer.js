...
// A node marker's radius as a fraction of the model. These used to be fixed
// world sizes, which meant a detail a couple of metres across was buried under
// its own nodes while a bridge showed none you could find. Everything drawn as
// a symbol rather than as structure — nodes, supports, springs, couplings — is
// sized from this one number, so one control moves them together.
const SYMBOL_RADIUS_FRACTION = 0.01;
...
// How far the size control may be taken either side of the size the model
// suggests, which is what "1" means.
const SYMBOL_SCALE_RANGE = [0.25, 4];
// A spring is drawn as the coil it is: this many zigzag half-waves across the
// middle of its length, with the ends left straight so it still reads as
// joining the two nodes it joins.
const SPRING_COILS = 6;
const SPRING_BODY = 0.6;
// A coupling is a rigid link, drawn as a plain line with a tick across each end
// — the mark that says the ends cannot move apart.
const COUPLING_TICK = 1.4;
...
class GravissRenderer {
  ...
  // A symbol's size in world units: taken from the model so it reads the same
  // whatever the structure measures, and then whatever the user asked for.
  symbolRadius() {
    return this.bounds.radius * SYMBOL_RADIUS_FRACTION * this.symbolScale;
  }
  ...
  // One size for everything drawn as a symbol rather than as structure.
  setSymbolScale(scale) {
    const [lowest, highest] = SYMBOL_SCALE_RANGE;
    const wanted = Number.isFinite(scale) ? scale : 1;
    const next = Math.min(highest, Math.max(lowest, wanted));
    if (next === this.symbolScale) return this.symbolScale;
    this.symbolScale = next;
    this.placeNodeSymbols();
  ...
  createConnectorGeometry(nodesById, kind, visibilityKey) {
    ...
    if (elements.length === 0) return;
    const perElement = kind === "spring" ? SPRING_COILS + 4 : 6;
  ...
  placeConnectorSymbols(kind) {
    ...
    for (const element of elements) {
      ...
      if (nodes.length > 1 && nodes[1]) {
        ...
        end.copy(start).addScaledVector(along.normalize(), size * SPRING_COILS);
      }
      ...
      // Any direction square to the axis will do; the coil has no roll of its
      // own, so the one that is furthest from the axis is the steadiest choice.
      ...
      if (kind === "spring") {
        ...
        const step = (length * SPRING_BODY) / SPRING_COILS;
        from.copy(start);
        to.copy(start).addScaledVector(along, bodyStart);
        segment(from, to);
        for (let coil = 0; coil < SPRING_COILS; coil += 1) {
          from.copy(to);
          to.copy(start)
            .addScaledVector(along, bodyStart + step * (coil + 1))
            .addScaledVector(across, coil % 2 === 0 ? 1 : -1);
          if (coil === SPRING_COILS - 1) to.addScaledVector(across, coil % 2 === 0 ? -1 : 1);
          segment(from, to);
        }
        segment(to, end);
        for (let spare = SPRING_COILS + 2; spare < perElement; spare += 1) segment(end, end);
      } else {
        segment(start, end);
        for (const anchor of [start, end]) {
          from.copy(anchor).addScaledVector(across, COUPLING_TICK);
          to.copy(anchor).addScaledVector(across, -COUPLING_TICK);
          segment(from, to);
        }
        for (let spare = 3; spare < perElement; spare += 1) segment(end, end);
      }
    }
