...
class GravissRenderer {
  ...
  keepsElement(element) {
    const switchName = GravissRenderer.MEMBER_VISIBILITY[element.kind];
    if (switchName && this.visibility[switchName] === false) return false;
  ...
  // The three member kinds, and the switch each one answers to. `members` stays
  // the umbrella over all of them: it is one object in the scene graph and it is
  // what every document written before these three existed says.
  //
  // A kind cannot be taken out of the group the way the umbrella is, because the
  // group is drawn instanced and `geometry.groups` splits a vertex buffer rather
  // than a set of instances - every instance draws every group. So a kind is
  // hidden the way a filter hides anything, by not being drawn at all, which is
  // also what makes the two compose into one pass instead of fighting.
  static MEMBER_VISIBILITY = Object.freeze({ beam: "beams", truss: "trusses", cable: "cables" });
  ...
  setVisibility(name, visible) {
    ...
    if (Object.values(GravissRenderer.MEMBER_VISIBILITY).includes(name)) {
      this.applyElementFilter();
      this.placeMemberLines();
      this.dropHiddenSelection();
      this.requestRender();
