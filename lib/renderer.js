...
class GravissRenderer {
  ...
  createReferenceGeometry() {
    ...
    // Everything drawable except the camera-following sky, as one sphere: the
    // model, the grid laid out around it, the axes standing at the origin.
    // From outside it the near plane may ride up to its front, which is what
    // keeps the depth test exact however far the camera backs away.
    const sceneCenter = new THREE.Vector3(...center);
    this.sceneSphere = {
      center: sceneCenter,
      radius: Math.max(
        radius,
        this.grid.position.distanceTo(sceneCenter) + gridSize * Math.SQRT2 * 0.5,
        sceneCenter.length() + axesSize,
      ),
    };
  }
  ...
  createShellGeometry(nodesById, elements) {
    ...
    const prepared = elements.map((element) => {
      ...
      const middle = elementOffsets(element, nodes.length, shared.offsets);
  ...
  updateDepthRange() {
    ...
    const clearance = this.sceneSphere
      ? this.camera.position.distanceTo(this.sceneSphere.center) - this.sceneSphere.radius
      : 0;
    const near = Math.max(
      far / MAXIMUM_DEPTH_RATIO,
      distance / 1000,
      Math.min(clearance * 0.9, far / 3),
    );
