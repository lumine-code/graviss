...
// How far the top of a graded background is lifted towards white. Enough to
// read as light falling from above and not so much that the model has to
// compete with the sky behind it.
const BACKGROUND_GRADIENT_LIFT = 0.34;
// A gradient needs no width and only enough height to be smooth; the card
// stretches one column across the viewport.
const BACKGROUND_GRADIENT_STEPS = 256;
...
class GravissRenderer {
  ...
  // A flat colour, or the same colour with the top lifted towards white — the
  // light a room has when it comes from above. The clear colour stays flat
  // either way: it is what shows where the background does not reach.
  applyBackground(appearance) {
    ...
    const lifted = base.clone().lerp(new THREE.Color(0xffffff), BACKGROUND_GRADIENT_LIFT);
    const canvas = document.createElement("canvas");
    canvas.width = 1;
    canvas.height = BACKGROUND_GRADIENT_STEPS;
    const context = canvas.getContext("2d");
    const gradient = context.createLinearGradient(0, 0, 0, canvas.height);
    gradient.addColorStop(0, `#${lifted.getHexString()}`);
