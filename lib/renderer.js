...
class GravissRenderer {
  ...
  createBeamGeometry(nodesById, elements) {
    ...
    for (const group of groups.values()) {
      ...
      // After the shell edges, which write depth: a member in front of a
      // slab's mesh lines then paints over the sliver its sunk fill would let
      // pierce along a grazing intersection, while lines genuinely in front
      // keep their pixels through the depth they wrote.
  ...
  createMemberContours(memberMesh) {
    ...
    // surface these arrises lie on, and drawn after the member fill.
