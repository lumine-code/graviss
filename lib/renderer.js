...
// definition. So the intermediate points are not read but worked out - here,
// per vertex, from three numbers an instance carries.
//
// Only the deviation from the chord. The two end nodes have already moved, and
// the instance matrix was written from where they moved to, so the straight
// part of the displacement is drawn already; what is left is the bow, which is
// zero at both ends by construction. That is what keeps this honest when a
// provider states no stations at all - the member is then simply straight
// between its moved ends, which is the best anyone can say about it.
//
// A single uniform says how far. The stations change when a load case does; the
// phase changes sixty times a second, and it costs one number.
...
class GravissRenderer {
  ...
  placeMemberLines() {
    ...
    lines.userData.gravissEntities.forEach((element, index) => {
      ...
      const at = index * 6;
      into[at] = nodes[first];
      into[at + 1] = nodes[first + 1];
      into[at + 2] = nodes[first + 2];
      into[at + 3] = nodes[second];
      into[at + 4] = nodes[second + 1];
      into[at + 5] = nodes[second + 2];
    });
