...
class GravissRenderer {
  ...
  shellLayerCount(element) {
    return this.elementDetail !== "axis" && element.thickness ? 2 : 1;
  }
  ...
  applyShellDetail() {
    ...
    if (!rebuilt) return;
    // An area element is always drawn as its area. Only the last level gives it
    // the thickness it was given; switching section rendering off takes the
    // extrusion away from line elements, not the surface away from area ones.
