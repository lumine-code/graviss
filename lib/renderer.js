...
const { PRINT_MARGIN_FRACTION, printPixelSize, validatePrintRegion } = require("./print-region");
...
class GravissRenderer {
  ...
  modelScreenRect() {
    ...
    for (const x of [min[0], max[0]]) {
      for (const y of [min[1], max[1]]) {
        for (const z of [min[2], max[2]]) {
          // A corner behind the camera projects to nonsense, so it is dropped
          // rather than allowed to blow the rectangle up.
          corner.set(x, y, z).applyMatrix4(this.camera.matrixWorldInverse);
          if (this.camera.isPerspectiveCamera && corner.z > -this.camera.near) continue;
  ...
  renderRegionCrop(region, options) {
    ...
    const pixels = printPixelSize(covered, options);
  ...
  renderModelFit(options) {
    ...
    if (!rect) return null;
    const margin = Math.max(rect.width, rect.height) * PRINT_MARGIN_FRACTION;
    return this.renderRegionCrop(
      {
        x: rect.x - margin,
        y: rect.y - margin,
