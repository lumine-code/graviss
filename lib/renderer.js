...
class GravissRenderer {
  ...
  createMemberGeometry(nodesById, elements) {
    ...
    for (const group of groups.values()) {
      ...
      this.bakeMemberFill(unit, matrices, group.elements, memberGroup, "element");
      memberGroup.add(this.createMemberContours(unit, matrices));
      unit.dispose();
      // The part of the section that does not carry, stamped over the member
      // it belongs to and drawn after it. The two surfaces are exactly
      // coincident — this is the same section, extruded through the same
      // matrices, over an area cut out of it — so the later draw wins every
      // tie outright and no offset or inflation is wanted anywhere. Its own
      // walls, which stand inside the member, lose to the member's surface in
      // front of them and stay hidden, which is what they should do.
  ...
  bakeMemberFill(unit, matrices, elements, memberGroup, colorKey) {
    const THREE = this.THREE;
    ...
    if (intersection) {
      ...
      const entityIndex =
        ...
        (intersection.object.userData.gravissLineSegments
          ? entityIndexAtVertex(
              intersection.object.userData.gravissEntityRanges,
              intersection.index,
            )
