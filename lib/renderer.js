...
class GravissRenderer {
  ...
  createBeamGeometry(nodesById, elements) {
    ...
    // Its own contours never rely on the tie the other way: transformed on
    // the CPU while the fill transforms on the GPU, they round differently,
    // so their profile is inflated a hair instead, in createMemberContours.
  ...
  createShellGeometry(nodesById, elements) {
    ...
    // line paints over it — and after the member fills, so a flange lying
    // exactly in this surface's plane has already claimed its band and the
    // tie-losing test leaves it there.
