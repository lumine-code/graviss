...
const { Deformation, memberBow } = require("./deformation");
const {
...
// The bow a member takes between its two ends, evaluated on the card.
//
// This is `memberBow` in lib/deformation.js transcribed into GLSL, because the
// card cannot call it. That module is the authority and its specs pin the shape;
// what is here besides is the turn of the section, which only a vertex needs.
//
// Only the deviation from the chord: the end nodes have already moved and the
// instance matrix was written from where they moved to, so the straight part of
// the displacement is drawn already. A single uniform says how far - the
// stations change when a load case does, and the phase changes sixty times a
// second for the cost of one number.
const MEMBER_BEND_CHUNK = `
  #ifdef GRAVISS_BEND
  if (gravissBendFactor != 0.0) {
    float bendLength = length(instanceMatrix[0].xyz);
    if (bendLength > 0.0) {
      float xi = clamp(transformed.x + 0.5, 0.0, 1.0);
      float xi2 = xi * xi;
      float xi3 = xi2 * xi;
      float h1 = 1.0 - 3.0 * xi2 + 2.0 * xi3;
      float h2 = xi - 2.0 * xi2 + xi3;
      float h3 = 3.0 * xi2 - 2.0 * xi3;
      float h4 = -xi2 + xi3;
      float g1 = -6.0 * xi + 6.0 * xi2;
      float g2 = 1.0 - 4.0 * xi + 3.0 * xi2;
      float g3 = 6.0 * xi - 6.0 * xi2;
      float g4 = -2.0 * xi + 3.0 * xi2;
      vec3 u0 = instanceBendA.xyz;
      vec3 u1 = instanceBendB.xyz;
      float ry0 = instanceBendR.x;
      float rz0 = instanceBendR.y;
      float ry1 = instanceBendR.z;
      float rz1 = instanceBendR.w;
      // A rotation about the local z axis lifts a point ahead of it toward +y,
      // and one about the local y axis pushes it toward -z. Right-handed, which
      // is what the contract says a local frame is.
      float curveY = h1 * u0.y + h2 * bendLength * rz0 + h3 * u1.y + h4 * bendLength * rz1;
      float curveZ = h1 * u0.z - h2 * bendLength * ry0 + h3 * u1.z - h4 * bendLength * ry1;
      float bowY = curveY - mix(u0.y, u1.y, xi);
      float bowZ = curveZ - mix(u0.z, u1.z, xi);
      float slopeY = (g1 * u0.y + g2 * bendLength * rz0 + g3 * u1.y + g4 * bendLength * rz1 - (u1.y - u0.y)) / bendLength;
      float slopeZ = (g1 * u0.z - g2 * bendLength * ry0 + g3 * u1.z - g4 * bendLength * ry1 - (u1.z - u0.z)) / bendLength;
      // The section turns with the bow it sits on and with the twist along the
      // member, or a bent beam reads as a sheared one. Small angles, which is
      // what a displacement result is.
      float twist = mix(instanceBendA.w, instanceBendB.w, xi);
      vec3 turn = vec3(twist, -slopeZ, slopeY) * gravissBendFactor;
      vec3 offset = vec3(0.0, transformed.y, transformed.z);
      vec3 turned = cross(turn, offset);
      // x is the one axis the instance matrix scales, so a length along it is
      // stated as a fraction of the member rather than in metres.
      transformed.x += turned.x / bendLength;
      transformed.y += bowY * gravissBendFactor + turned.y;
      transformed.z += bowZ * gravissBendFactor + turned.z;
    }
  }
  #endif
`;
...
class GravissRenderer {
  ...
  initialize() {
    ...
    this.canvasRenderer.setPixelRatio(Math.min(this.window.devicePixelRatio || 1, 2));
  ...
  colorFromTheme(variable, fallback) {
    ...
    const resolved = this.window.getComputedStyle(probe).color;
    probe.remove();
  ...
  keepsElement(element) {
    return this.elementFilter ? this.elementFilter(element) : true;
  }
  ...
  setDeformationPhase(phase) {
    if (!this.deformation) return null;
    this.deformation.setPhase(phase);
    this.applyDeformation();
    return phase;
  }
  ...
  applyDeformation() {
    ...
    if (this.elementFilter) this.applyElementFilter();
    for (const mesh of this.pickables) {
      if (mesh.isInstancedMesh && mesh.userData.gravissInstanced) refreshInstanceBounds(mesh);
    }
    this.placeMemberLines();
    this.updateShellSurface();
  ...
  measureDeformedBox() {
    ...
    for (let at = 0; at + 2 < positions.length; at += 3) {
      box.expandByPoint(point.set(positions[at], positions[at + 1], positions[at + 2]));
    }
  ...
  placeMemberLines() {
    ...
    lines.userData.gravissEntities.forEach((element, index) => {
      ...
      this.memberAxes(element, runX, runY, runZ, xAxis, yAxis, zAxis);
      const length = Math.hypot(runX, runY, runZ);
      const place = (fraction, offset) => {
        const bow = memberBow(stations, length, fraction);
        into[offset] = startX + runX * fraction + (bow.y * yAxis.x + bow.z * zAxis.x) * factor;
        into[offset + 1] = startY + runY * fraction + (bow.y * yAxis.y + bow.z * zAxis.y) * factor;
        into[offset + 2] = startZ + runZ * fraction + (bow.y * yAxis.z + bow.z * zAxis.z) * factor;
      };
      for (let step = 0; step < steps; step += 1) {
        place(step / steps, at);
        place((step + 1) / steps, at + 3);
        at += 6;
      }
    });
  ...
  createMemberGeometry(nodesById, elements) {
    ...
    for (const group of groups.values()) {
      ...
      const arris = this.createMemberContours(unit, placement);
      ...
      const ineffective = this.createIneffectiveGeometry(group.section?.ineffective);
      if (ineffective) {
  ...
  memberPlacement(elements) {
    ...
    // needs. Three vectors an instance: the two end displacements with the twist
    // at each end in the spare slot, and the two end rotations that are not the
    // twist. Written when a result arrives, not per frame.
  ...
  writeMemberStations(placement) {
    ...
    const { bendA, bendB, bendR } = placement;
    bendA.array.fill(0);
    bendB.array.fill(0);
    bendR.array.fill(0);
    if (stations) {
  ...
  createIneffectiveGeometry(areas) {
  ...
  createMemberContours(unit, placement) {
    const THREE = this.THREE;
  ...
  createSectionGeometry(shape, steps = 1) {
    ...
    if (shape.kind === "rectangle") {
      return new THREE.BoxGeometry(1, shape.width, shape.height, steps, 1, 1);
    }
    ...
    if (shape.kind === "tube") {
      ...
      return this.extrudeSectionShape(outline, 16, steps);
    }
    if (shape.kind === "tee") {
      const outline = new THREE.Shape();
      ...
      outline.lineTo(-halfFlange, flangeBottom);
      outline.closePath();
      return this.extrudeSectionShape(outline, 1, steps);
    }
    if (shape.kind === "polygon") {
      ...
      return this.extrudeSectionShape(outlines.length === 1 ? outlines[0] : outlines, 8, steps);
    }
    if (shape.kind === "plates") {
      ...
      return this.extrudeSectionShape(outlines.length === 1 ? outlines[0] : outlines, 1, steps);
    }
  ...
  writeShellSurface(state, { entityRanges = null, faceToEntityIndex = null } = {}) {
    const { prepared, positions, edgePositions } = state;
    let vertexCursor = 0;
    let edgeCursor = 0;
    let faceCursor = 0;
  ...
  updateShellNormals(state) {
    ...
    const indices = state.nodeIndices;
    const sums = state.normalSums;
    const own = state.ownNormals;
    sums.fill(0);
    let corner = 0;
    for (let item = 0; item < prepared.length; item += 1) {
      const nodes = prepared[item].nodes;
      for (let at = 0; at < nodes.length; at += 1, corner += 1) {
        const normal = cornerNormalAt(nodes, at);
        const to = corner * 3;
        if (!normal) {
          own[to] = 0;
          own[to + 1] = 0;
        ...
        own[to + 1] = normal.y;
        own[to + 2] = normal.z;
        const node = indices[corner];
        if (node < 0) continue;
        sums[node * 3] += normal.x;
        sums[node * 3 + 1] += normal.y;
        sums[node * 3 + 2] += normal.z;
      }
    }
    // Written back into the vectors that are already there. A fresh object a
    // corner would be twenty-five thousand allocations a frame, which the
    // collector pays for in exactly the pauses an animation is judged by.
    corner = 0;
    for (let item = 0; item < prepared.length; item += 1) {
      const normals = prepared[item].normals;
      for (let at = 0; at < normals.length; at += 1, corner += 1) {
        const from = corner * 3;
        const ownX = own[from];
        const ownY = own[from + 1];
        const ownZ = own[from + 2];
        ...
        const node = indices[corner];
        if (node < 0) continue;
        const sumX = sums[node * 3];
        const sumY = sums[node * 3 + 1];
        const sumZ = sums[node * 3 + 2];
        const length = Math.hypot(sumX, sumY, sumZ);
        if (!(length > 1e-6)) continue;
        const meanX = sumX / length;
        const meanY = sumY / length;
        const meanZ = sumZ / length;
        // A mean pointing away from the corner's own normal is a seam between
        // surfaces that meet at an angle, not one surface bending: taking it
        // would fold the element through itself.
        if (meanX * ownX + meanY * ownY + meanZ * ownZ <= 1e-6) continue;
        target.x = meanX;
        target.y = meanY;
        target.z = meanZ;
      }
  ...
  createShellGeometry(nodesById, elements) {
    ...
    this.shellState = {
      ...
      elements,
      nodeIndices,
      normalSums: new Float32Array(this.geometry.nodes.length * 3),
      ownNormals: new Float32Array(cornerCount * 3),
    };
  ...
  resolveAppearance(appearance) {
    const styles = this.window.getComputedStyle(this.host);
  ...
  didChangeSurface() {
    ...
    this.canvasRenderer.setPixelRatio(Math.min(this.window.devicePixelRatio || 1, 2));
    this.viewCube?.didChangeSurface();
    this.observeSize();
    this.requestRender();
  }
  ...
  beginWindowSurfaceTransition() {
    ...
    if (this.renderFrame != null) this.renderFrameWindow?.cancelAnimationFrame(this.renderFrame);
    this.renderFrame = null;
    this.renderFrameWindow = null;
    this.resizeObserver?.disconnect();
    this.resizeWindow?.removeEventListener("resize", this.onWindowResize);
  ...
  beginRegionSelection(onComplete) {
    ...
    canvas.addEventListener("keydown", selection.onKeyDown, true);
    return true;
  }
  ...
  endRegionSelection(region = null) {
    ...
    canvas.removeEventListener("keydown", selection.onKeyDown, true);
  ...
  cancelZoomFlight() {
    ...
    if (this.zoomFrame != null) this.zoomFrameWindow?.cancelAnimationFrame(this.zoomFrame);
    this.zoomFrame = null;
    this.zoomFrameWindow = null;
    this.zoomFlight = null;
    this.zoomFrameTime = null;
  ...
  advanceZoomFlight(timestamp) {
    ...
    this.scheduleCameraChange();
    this.zoomFrameWindow = this.window;
    this.zoomFrame = this.zoomFrameWindow.requestAnimationFrame((next) =>
      this.advanceZoomFlight(next),
    );
  }
  ...
  animateToFittedView(fittedView) {
    ...
    const step = (timestamp) => {
      ...
      if (progress < 1) {
        this.cameraAnimationWindow = this.window;
        this.cameraAnimationFrame = this.cameraAnimationWindow.requestAnimationFrame(step);
        return;
      }
  ...
  cancelCameraAnimation() {
    ...
    if (this.cameraAnimationFrame != null) {
      this.cameraAnimationWindow?.cancelAnimationFrame(this.cameraAnimationFrame);
    }
    this.cameraAnimationFrame = null;
    this.cameraAnimationWindow = null;
    this.cameraAnimationTarget = null;
  ...
  requestRender() {
    ...
    this.renderFrameWindow = this.window;
    this.renderFrame = this.renderFrameWindow.requestAnimationFrame((timestamp) => {
      this.renderFrame = null;
      this.renderFrameWindow = null;
