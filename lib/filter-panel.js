...
class FilterRuleRow {
  constructor(panel, id) {
    ...
    const element = panel.element.ownerDocument.createElement("li");
    ...
    element.innerHTML = `
      ...
      <select class="input-select graviss-rule-type" aria-label="Dimension"></select>
    ...
    this.select = element.querySelector(".graviss-rule-type");
    this.field = element.querySelector(".graviss-rule-text");
    this.count = element.querySelector(".graviss-rule-count");

    this.select.addEventListener("change", () => this.chooseSubject());
  ...
  update(rule, named) {
    ...
    if (this.field.value !== rule.text && this.field.ownerDocument.activeElement !== this.field) {
  ...
  renderOptions(subjects, rule, subject) {
    ...
    if (rule.type && !subject) {
      ...
      missing.value = "";
      missing.textContent = `${rule.type} (not in this model)`;
    ...
    if (!chosen) this.select.selectedIndex = rule.type ? 1 : 0;
  }
