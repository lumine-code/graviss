...
class ResultsPanel extends GravissPanel {
  ...
  buildScalePresets() {
    ...
    auto.type = "button";
    auto.className = "btn btn-sm graviss-scale-auto";
  ...
  renderScale(state) {
    ...
    if (this.element.ownerDocument.activeElement !== this.scaleSlider) {
  ...
  renderAnimation(state) {
    ...
    if (this.element.ownerDocument.activeElement !== this.periodSlider) {
      this.periodSlider.value = String(period);
    }
  }
