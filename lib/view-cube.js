...
// The face border and its label share one texture, so the cube gets crisp inset
// edges without a second draw call per face and both parts recolour together
// with the scheme. The border is drawn at partial alpha so it reads as a rule
...
function createLabelTexture(THREE, label, anisotropy = 1) {
  ...
  // The contour traces the boundary between the face region and the edge
  // regions around it, so a hover highlight fills exactly one side of it. Any
  // other inset draws a rule the highlight cannot line up with, and square
  // corners because the corner regions are squares.
