...
class ViewCube {
  constructor(THREE, canvas, { coordinateSystem = null, onSelect } = {}) {
    const domWindow = canvas?.ownerDocument?.defaultView;
    if (!domWindow || !(canvas instanceof domWindow.HTMLCanvasElement)) {
    ...
    this.renderer.setPixelRatio(viewCubePixelRatio(domWindow.devicePixelRatio));
    ...
    this.scene.add(new THREE.HemisphereLight(0xffffff, 0x8ba1ad, 1.15));
  ...
  observeSize() {
    this.resizeObserver?.disconnect();
    const domWindow = this.canvas.ownerDocument.defaultView;
    this.resizeObserver = new domWindow.ResizeObserver(() => this.resize());
  ...
  createFace(definition) {
    ...
    const label = new THREE.Mesh(new THREE.PlaneGeometry(FACE_SIZE, FACE_SIZE), labelMaterial);
...
// The face contour and its label share one texture, so the cube gets crisp
// region rules without a second draw call per face and both parts recolour
// together with the scheme.
function createLabelTexture(THREE, label, anisotropy = 1, document = globalThis.document) {
  const canvas = document.createElement("canvas");
  canvas.width = LABEL_TEXTURE_SIZE;
  canvas.height = LABEL_TEXTURE_SIZE;
  const context = canvas.getContext("2d");
  context.clearRect(0, 0, LABEL_TEXTURE_SIZE, LABEL_TEXTURE_SIZE);
  // Every face is picked as nine regions: one face, four edges, four corners.
  // Two rules each way draw all of their boundaries, so a hover highlight of
  // any kind fills exactly one cell. Drawing only the inner square would leave
  // the corners running into the edges beside them with nothing between.
  // The inset comes from the constant that lays the regions out, and the
  // corners are square because the corner regions are.
  const inset = LABEL_TEXTURE_SIZE * (FACE_BORDER_SIZE / FACE_SIZE);
  const span = LABEL_TEXTURE_SIZE - inset * 2;
  const center = LABEL_TEXTURE_SIZE / 2;
  context.strokeStyle = "rgba(255, 255, 255, 0.55)";
  context.lineWidth = 10;
  context.beginPath();
  for (const offset of [inset, LABEL_TEXTURE_SIZE - inset]) {
    context.moveTo(offset, 0);
    context.lineTo(offset, LABEL_TEXTURE_SIZE);
    context.moveTo(0, offset);
    context.lineTo(LABEL_TEXTURE_SIZE, offset);
  }
  context.stroke();
  context.fillStyle = "#ffffff";
  context.font = `800 ${Math.round(span * 0.42)}px "Segoe UI", sans-serif`;
  context.textAlign = "center";
  context.textBaseline = "middle";
  context.fillText(label.toUpperCase(), center, center + 6, span * 0.84);
