...
class ViewCube {
  ...
  setScheme(name, palette = appearanceDefinition(name)) {
    this.scheme = name;
  ...
  paintTargets() {
    const palette = this.palette || appearanceDefinition(this.scheme);
    ...
    for (const target of this.displayTargets.values()) {
      ...
      for (const displayObject of target.displayObjects) {
        displayObject.material.color.setHex(highlighted ? palette.cubeHover : palette.cubeSelected);
