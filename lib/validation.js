...
// The six words a provider may say an element is, in one place, because a
// filter type may declare which of them its dimension is ever about.
const ELEMENT_KINDS = new Set(ELEMENT_NODE_COUNTS.keys());
...
function validateCapabilities(capabilities) {
  ...
  if (capabilities.filterTypes != null && capabilities.filterTypes !== true) {
    fail("description.capabilities.filterTypes must be true when supplied");
  }
  return capabilities;
}
...
function validateFilterTypes(filterTypes, location = "geometry.filterTypes") {
  if (!Array.isArray(filterTypes)) fail(`${location} must be an array when supplied`);
  const typeIds = new Set();
  const byType = new Map();
  filterTypes.forEach((type, index) => {
    const at = `${location}[${index}]`;
    if (!isObject(type)) fail(`${at} must be an object`);
    // A type id keys a rule in a view document, which round-trips through JSON,
    // so it is a string rather than an Id - there is no boundary here for a
    // number to survive.
    ...
    if (type.id.startsWith("@")) {
      fail(
        `${at}.id ${JSON.stringify(type.id)} must not begin with "@", which is reserved to Graviss`,
      );
    }
    if (typeIds.has(type.id)) fail(`${at}.id ${JSON.stringify(type.id)} is not unique`);
    typeIds.add(type.id);
    if (typeof type.title !== "string" || type.title.length === 0) {
      fail(`${at}.title must be a non-empty string`);
    }
    for (const flag of ["numeric", "multiple"]) {
      if (type[flag] != null && typeof type[flag] !== "boolean") {
        fail(`${at}.${flag} must be a boolean when supplied`);
      }
    }
    if (type.hint != null && (typeof type.hint !== "string" || !type.hint)) {
      fail(`${at}.hint must be a non-empty string when supplied`);
    }
    if (type.kinds != null) {
      if (!Array.isArray(type.kinds) || type.kinds.length === 0) {
        fail(`${at}.kinds must be a non-empty array when supplied`);
      }
      for (const kind of type.kinds) {
        if (!ELEMENT_KINDS.has(kind)) {
          fail(`${at}.kinds contains unknown element kind ${JSON.stringify(kind)}`);
        }
      }
    }
    // Values are optional. A dimension whose values are numbers needs no list -
    // a range says what it means without one - and a source with hundreds of
    // them should not be made to enumerate them for a viewer that will only
    // offer them as completions.
    let valueIds = null;
    if (type.values != null) {
      if (!Array.isArray(type.values)) fail(`${at}.values must be an array when supplied`);
      valueIds = new Set();
      type.values.forEach((value, valueIndex) => {
        ...
        if (type.numeric && !Number.isFinite(value.id)) {
          fail(`${valueAt}.id must be a finite number, because its type is numeric`);
        }
      });
    }
    byType.set(type.id, {
...
// dimension an element is silent about simply does not filter it.
function validateFilterValues(filterValues, byType, location) {
  if (!isObject(filterValues)) fail(`${location} must be an object when supplied`);
  for (const [typeId, held] of Object.entries(filterValues)) {
    const at = `${location}[${JSON.stringify(typeId)}]`;
    const type = byType.get(typeId);
    if (!type) fail(`${at} references unknown filter type ${JSON.stringify(typeId)}`);
    const list = Array.isArray(held) ? held : [held];
    if (Array.isArray(held) && !type.multiple) {
      fail(`${at} must be one value, because its filter type does not declare multiple`);
    }
    ...
    for (const value of list) {
      assertId(value, at);
      if (type.numeric && !Number.isFinite(value)) {
        fail(`${at} must hold finite numbers, because its filter type is numeric`);
      }
      // Only where the source enumerated its values is there anything to check
      // an element against.
      if (type.values && !type.values.has(`${typeof value}:${value}`)) {
...
function validateGeometry(geometry) {
  ...
  const filterTypes =
    geometry.filterTypes == null ? new Map() : validateFilterTypes(geometry.filterTypes);
  ...
  geometry.elements.forEach((element, index) => {
    ...
    if (element.filterValues != null) {
      validateFilterValues(element.filterValues, filterTypes, `${location}.filterValues`);
    }
