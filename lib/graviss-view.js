...
class GravissView {
    ...
    element.innerHTML = `
      <header class="graviss-toolbar btn-toolbar">
        ...
        <div class="block graviss-toolbar-block graviss-element-detail">
          <div class="btn-group btn-group-sm" role="group" aria-label="Element detail">
            <button type="button" class="btn graviss-toolbar-button graviss-detail-button" data-action="element-detail" data-detail="section" data-command="graviss:cycle-element-detail" aria-label="Draw elements as section">${toolbarIcon("detail-axis")}${toolbarIcon("detail-section")}${toolbarIcon("detail-full")}</button>
          </div>
        </div>
  ...
  // The button shows the level on screen rather than the one a click would
  // reach, so the toolbar reads as the state of the model and not as a verb.
  updateElementDetailControl(level = this.getElementDetail(), root = this.element) {
    const button = root.querySelector('[data-action="element-detail"]');
    if (!button || !DETAIL_LEVELS.includes(level)) return;
    button.dataset.detail = level;
    button.setAttribute("aria-label", `Draw elements as ${level}`);
  }
