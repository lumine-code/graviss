...
class GravissView {
      ...
      <header class="graviss-toolbar btn-toolbar">
        ...
        <div class="graviss-toolbar-region graviss-region-layers" role="group" aria-label="Layers">
          ...
          <div class="block graviss-toolbar-block graviss-symbol-size">
            <input type="number" class="input-text graviss-symbol-input" min="0" max="1000" step="5" aria-label="Symbol size in millimetres">
          </div>
        </div>
        <div class="graviss-toolbar-region graviss-toolbar-tail" role="group" aria-label="Document and renderer">
  ...
  bindInterface() {
    ...
    this.onSymbolWheel = (event) => {
      event.preventDefault();
      const step = (Number(sizeInput.step) / 1000) * (event.shiftKey ? 10 : 1);
