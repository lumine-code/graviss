...
// How much of an element is drawn. The cycle is the one a hand reaches for;
// each level also has its own command, for a keymap or the palette.
const DETAIL_COMMANDS = Object.freeze({
  "graviss:element-detail-axis": "axis",
  "graviss:element-detail-section": "section",
  "graviss:element-detail-full": "full",
});
...
class GravissView {
  ...
  createElement() {
    ...
    element.innerHTML = `
      <header class="graviss-toolbar btn-toolbar">
        ...
        <div class="block graviss-toolbar-block graviss-element-detail">
          <div class="btn-group btn-group-sm" role="group" aria-label="Section rendering">
            <button type="button" class="btn graviss-toolbar-button selected" data-action="toggle-sections" data-command="graviss:toggle-sections" aria-label="Draw elements without their sections" aria-pressed="true">${toolbarIcon("detail-section")}</button>
          </div>
        </div>
  ...
  createViewerCommands() {
    ...
    commands["graviss:cycle-element-detail"] = viewerCommand("Cycle Element Detail", () =>
      this.performViewerAction(() => this.cycleElementDetail()),
    );
    ...
    for (const [commandName, level] of Object.entries(DETAIL_COMMANDS)) {
      commands[commandName] = viewerCommand(`Draw Elements as ${titleForId(level)}`, () =>
        this.performViewerAction(() => this.setElementDetail(level)),
      );
    }
  ...
  applyActiveGraphic() {
    ...
    this.updateElementDetailControl();
  ...
  // How much of an element is drawn: axis, section, or full.
  setElementDetail(level) {
    const applied = this.renderer?.setElementDetail(level) ?? level;
    this.updateElementDetailControl(applied);
    return applied;
  }

  getElementDetail() {
    return this.renderer?.getElementDetail() ?? "section";
  }

  cycleElementDetail() {
    const levels = DETAIL_LEVELS;
    const next = levels[(levels.indexOf(this.getElementDetail()) + 1) % levels.length];
    return this.setElementDetail(next);
  }

  // Section rendering is the one distinction the toolbar makes: either line
  // elements carry their cross-section or they are drawn as lines. The `full`
  // level, which also gives area elements their thickness, counts as rendered
  // and stays reachable through its own command.
  isSectionRenderingEnabled() {
    return this.getElementDetail() !== "axis";
  }

  toggleSectionRendering() {
    return this.setElementDetail(this.isSectionRenderingEnabled() ? "axis" : "section");
  }

  updateElementDetailControl(level = this.getElementDetail(), root = this.element) {
    const button = root.querySelector('[data-action="toggle-sections"]');
    if (!button || !DETAIL_LEVELS.includes(level)) return;
    const rendered = level !== "axis";
    button.dataset.detail = level;
    button.classList.toggle("selected", rendered);
    button.setAttribute("aria-pressed", String(rendered));
    button.setAttribute(
      "aria-label",
      rendered ? "Draw elements without their sections" : "Draw elements with their sections",
    );
  }
