...
class GravissView {
    ...
    element.innerHTML = `
      <header class="graviss-toolbar btn-toolbar">
        ...
        <div class="block graviss-toolbar-block graviss-symbol-size">
          <input type="number" class="input-text graviss-symbol-input" min="0" max="1" step="0.005" aria-label="Symbol size in metres">
        </div>
  ...
  bindInterface() {
    ...
    this.onSymbolInput = (event) => this.setSymbolSize(Number(event.target.value));
    // A size is a length, and a length is easier turned than typed: the wheel
    // over the field steps it by whatever the field itself steps by, and a
    // hundredth of that with the modifier held for a size worth being fussy
    // about.
    this.onSymbolWheel = (event) => {
      event.preventDefault();
      const step = Number(sizeInput.step) * (event.shiftKey ? 10 : 1);
  ...
  // Shown to the millimetre, which is the precision a mark on a structure is
  // worth stating to and what the field steps by.
  updateSymbolSizeControl(size) {
    const input = this.element.querySelector(".graviss-symbol-input");
    if (!input) return size;
    const shown = String(Number(size.toFixed(4)));
