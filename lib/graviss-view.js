...
class GravissView {
  ...
  showBackgroundList() {
    if (this.destroyed || this.backgroundList?.isVisible()) return;
    if (!this.backgroundList) {
      ...
      this.subscriptions.add(this.backgroundList.onDidHide(() => this.didHideBackgroundList()));
    }
    this.backgroundList.setItems(BACKGROUND_OPTIONS);
    this.backgroundList.selectItemById(this.appearance);
