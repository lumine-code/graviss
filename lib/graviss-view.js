...
const { GravissViewDocument, graphicAt } = require("./view-document");
...
class GravissView {
    ...
    element.innerHTML = `
      <header class="graviss-toolbar btn-toolbar">
        ...
        <div class="block graviss-toolbar-block graviss-symbol-size">
          <input type="range" class="input-range graviss-symbol-slider" min="0.25" max="4" step="0.05" value="1" aria-label="Symbol size">
        </div>
  ...
  bindInterface() {
    ...
    this.onSymbolSlider = (event) => this.setSymbolScale(Number(event.target.value));
    this.element
      .querySelector(".graviss-symbol-slider")
      .addEventListener("input", this.onSymbolSlider);
  ...
  createViewerCommands() {
    ...
    for (const [commandName, viewId] of Object.entries(VIEW_COMMANDS)) {
      commands[commandName] = viewerCommand((event) =>
  ...
  applyActiveGraphic() {
    ...
    this.applySymbolScale(graphic.symbolScale);
  ...
  setSymbolScale(scale) {
    const applied = this.renderer?.setSymbolScale(scale) ?? scale;
    this.updateSymbolScaleControl(applied);
    this.updateActiveGraphic((graphic) => {
      graphic.symbolScale = applied;
    }, "symbol-scale");
    return applied;
  }
  ...
  applySymbolScale(scale) {
    const applied = this.renderer?.setSymbolScale(scale ?? 1) ?? 1;
    this.updateSymbolScaleControl(applied);
    return applied;
  }

  updateSymbolScaleControl(scale) {
    const slider = this.element.querySelector(".graviss-symbol-slider");
  ...
  destroy() {
    ...
    this.element
      .querySelector(".graviss-symbol-slider")
      ?.removeEventListener("input", this.onSymbolSlider);
