...
class GravissView {
  constructor(session, options = {}) {
    ...
    this.bindInterface();
    this.observePaneSurface();
    this.load();
  }

  observePaneSurface() {
    const observe = lumine.workspace.observePaneItemSurface;
    if (typeof observe !== "function") return;
    this.subscriptions.add(observe.call(lumine.workspace, this, () => this.didChangePaneSurface()));
  }

  didChangePaneSurface() {
    this.rebindPrintRegionWindow?.();
    this.renderer?.didChangeSurface();
    this.updatePrintRegionOverlay();
  }

  beginWindowSurfaceTransition() {
    this.renderer?.beginWindowSurfaceTransition();
    const finish = () => this.didChangePaneSurface();
  ...
  bindPrintRegionInteraction() {
    ...
    const onPointerDown = (event) => {
      ...
      drag = {
        ...
        bounds,
        window: viewport.ownerDocument.defaultView,
      };
      overlay.classList.add("is-dragging");
      showCursor();
      drag.window.addEventListener("pointermove", onPointerMove, true);
      drag.window.addEventListener("pointerup", onPointerUp, true);
      drag.window.addEventListener("pointercancel", onPointerUp, true);
    };
    ...
    const onPointerUp = (_event) => {
      ...
      const dragWindow = drag.window;
      drag = null;
      dragWindow.removeEventListener("pointermove", onPointerMove, true);
      dragWindow.removeEventListener("pointerup", onPointerUp, true);
      dragWindow.removeEventListener("pointercancel", onPointerUp, true);
    ...
    this.rebindPrintRegionWindow = () => {
      this.printRegionWindowDisposable?.dispose();
      const domWindow = this.element.ownerDocument.defaultView;
      domWindow.addEventListener("keydown", onModifierChange, true);
      domWindow.addEventListener("keyup", onModifierChange, true);
      domWindow.addEventListener("blur", forgetPointer);
      this.printRegionWindowDisposable = new Disposable(() => {
        domWindow.removeEventListener("keydown", onModifierChange, true);
        domWindow.removeEventListener("keyup", onModifierChange, true);
        domWindow.removeEventListener("blur", forgetPointer);
      });
    };
    this.rebindPrintRegionWindow();
    this.subscriptions.add(
      new Disposable(() => {
        this.printRegionWindowDisposable?.dispose();
        this.printRegionWindowDisposable = null;
        this.rebindPrintRegionWindow = null;
      }),
    );
  }
  ...
  async saveAsImage() {
    ...
    const chosen = await lumine.workspace.showSaveDialogForPaneItem(this, {
