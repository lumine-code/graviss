...
const { createElementFilter } = require("./element-filter");
...
const DEFAULT_FILTER_STATE = Object.freeze({ numbers: "", facets: {} });
...
class GravissView {
  ...
  getFilterState() {
    return { numbers: this.filterState.numbers, facets: { ...this.filterState.facets } };
  }

  getFacets() {
    return this.geometry?.facets ?? [];
  }
  ...
  applyFilterState(state, { record = true } = {}) {
    this.filterState = {
      numbers: typeof state?.numbers === "string" ? state.numbers : "",
      facets: { ...(state?.facets || {}) },
    };
    if (this.renderer) {
      try {
        this.renderer.setElementFilter(createElementFilter(this.filterState));
      } catch {
        // An expression half typed is not an expression yet. The field says so
        // itself, and narrowing the model to nothing mid-word would be answering
  ...
  recordFilterState() {
    const stated = {};
    if (this.filterState.numbers) stated.numbers = this.filterState.numbers;
    const facets = Object.fromEntries(
      Object.entries(this.filterState.facets).filter(([, values]) => values?.length),
    );
    if (Object.keys(facets).length) stated.facets = facets;
    this.updateActiveGraphic((graphic) => {
      if (Object.keys(stated).length) graphic.filter = stated;
      else delete graphic.filter;
    }, "filter");
  }

  setNumberFilter(numbers) {
    return this.applyFilterState({ ...this.getFilterState(), numbers });
  }

  setFacetFilter(facetId, values) {
    const facets = { ...this.filterState.facets, [facetId]: values ?? [] };
    return this.applyFilterState({ ...this.getFilterState(), facets });
  }
