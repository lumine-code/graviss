...
class GravissView {
  ...
  bindInterface() {
    ...
    this.subscriptions.add(
      ...
      lumine.config.observe("graviss.orbitAroundPointer", () => this.applyOrbitPivotSettings()),
      lumine.config.observe("graviss.showOrbitPivot", () => this.applyOrbitPivotSettings()),
    );
    this.bindTooltips();
  ...
  // How the pointer turns the model is a preference, not something a graphic
  ...
  bindPrintRegionInteraction() {
    ...
    const onWheel = (event) => {
      if (!this.selectionMode && !this.modifierHeld) return;
      claim(event);
    };
  ...
  async saveAsImage() {
    ...
    const suggested = `${this.getTitle() || "graviss"}.png`;
    const directory = this.getPath() ? path.dirname(this.getPath()) : undefined;
