...
function normalizeViewDocument(document) {
  ...
  const graphics = (declared.length ? declared : [{}]).map((graphic, index) => ({
    ...
    ...pick("symbolScale", graphic.symbolScale, isSymbolScale),
...
// A multiplier on the size the model suggests, so it is a positive number and
// the renderer holds it to the range it will actually draw.
function isSymbolScale(value) {
