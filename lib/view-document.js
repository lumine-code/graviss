...
const { CompositeDisposable, Emitter, FileState, TextBuffer } = require("lumine");
...
class GravissViewDocument {
  constructor({
    ...
    const restoredFileState = restoredState?.fileState;
    this.restoredFileState =
      restoredFileState === FileState.CONFLICTED ||
      restoredFileState === FileState.REMOVED ||
      (!filePath && restoredFileState === FileState.MODIFIED)
        ? restoredFileState
        : null;
    ...
    this.lastReportedFileState = this.getFileState();
  ...
  subscribeToSourceBuffer() {
    ...
    this.sourceSubscriptions.add(
      ...
      buffer.onDidReload(() => {
        this.reloading = false;
        this.clearRestoredFileState();
        if (this.applySourceText("reload")) this.emitter.emit("did-reload", this.data);
      }),
      buffer.onDidChangeFileState((fileState) => {
        if (fileState === FileState.CONFLICTED || fileState === FileState.REMOVED) {
          this.restoredFileState = null;
        }
        this.publishFileState();
      }),
  ...
  getFileState() {
    return this.restoredFileState || this.sourceBuffer?.getFileState() || FileState.UNMODIFIED;
  }
  ...
  clearRestoredFileState() {
    if (!this.restoredFileState) return;
    this.restoredFileState = null;
    this.publishFileState();
  }

  publishFileState() {
    const next = this.getFileState();
    if (next === this.lastReportedFileState) return;
    this.lastReportedFileState = next;
    this.emitter.emit("did-change-file-state", next);
  }
