...
// A rectangle in metres on the plane through the camera target: its centre
// offset along the camera's right and up axes, and its extents. Metres,
// because a plot area has to name the same piece of the structure in any
// pane and from a batch render — fractions of a window never could.
function isPrintRegion(value) {
  if (!isObject(value)) return false;
  const { right, up, width, height } = value;
  return (
