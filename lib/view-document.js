...
// Which elements a user narrowed the model down to. The kinds are not here -
// they are switches like every other switch, and live in `visibility` - so what
// is left is the number expression as it was typed and whichever of a source's
// own dimensions were narrowed by.
//
// The expression is kept verbatim rather than as whatever it parsed to. It is
// what the user wrote and what the field has to show them again, and a source
// that renumbers between two openings should filter by what was asked for, not
// by the numbers it meant last time.
function isFilter(value) {
  if (!isObject(value)) return false;
  if (value.numbers != null && typeof value.numbers !== "string") return false;
  if (value.facets != null) {
    if (!isObject(value.facets)) return false;
    if (!Object.values(value.facets).every((values) => Array.isArray(values) && values.every(isId)))
      return false;
  }
  return true;
}
