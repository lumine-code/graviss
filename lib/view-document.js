...
function parseViewDocumentText(text) {
  ...
  if (!text.trim()) {
    return { data: createDefaultViewDocument(), implicit: true };
  }
  return { data: validateViewDocument(JSON.parse(text)), implicit: false };
}

// No title: a new document is named after the file it is saved as, and writing
// the file's own name into it as data would only be a copy to keep in step.
function createDefaultViewDocument() {
  return {
    format: "graviss-view",
    version: 1,
