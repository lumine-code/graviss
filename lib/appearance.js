// One blue engineering family across every scheme, with real tonal separation
// between the surface elements, the members extruded from them, and the mesh
// lines that describe both. A beam is the `member` colour itself; a truss and a
// cable step off it by hue rather than by lightness, far enough to be told
// apart in a model that mixes all three and close enough that they still read
// as the same thing carrying load differently. The one colour outside the family is `ineffective`,
// a grey: the part of a section that does not carry is the one thing on screen
// that is not structure, and saying so by draining the colour out of it reads
// at a glance and needs no legend. The schemes differ in the background they sit on
// and in how far the family has to move to stay readable against it.
//
// The family sits light. A model is mostly its own elements, and a dark one
// reads as a mass of ink rather than as a structure standing in a room — the
// edge lines carry the definition instead, which is why they are the one part
// that stayed dark.

const APPEARANCES = Object.freeze({
  cloud: Object.freeze({
    label: "Cloud",
    background: 0xdcedfa,
    member: 0x5e8bb1,
    truss: 0x4f93a4,
    cable: 0x7d7ab8,
    ineffective: 0x9aa4ac,
    shell: 0x85b2d7,
    shellEdge: 0x2a4359,
    node: 0x61b571,
    support: 0x3dbeee,
    spring: 0x76c043,
    coupling: 0xe08a4a,
    selected: 0xff6b35,
    grid: 0xa3bcca,
    gridCenter: 0x6d8a9a,
    cubeFace: 0xf4f9fd,
    cubeBorder: 0x4a7ea6,
    cubeText: 0x1d4b6d,
    cubeHover: 0x76d5f2,
    cubeSelected: 0x25b8e8,
  }),
  midnight: Object.freeze({
    label: "Midnight",
    background: 0x101b24,
    member: 0xaad2ef,
    truss: 0x8ad2d6,
    cable: 0xbbb4ef,
    ineffective: 0x7d858c,
    shell: 0x6995b6,
    shellEdge: 0xc4dbe9,
    node: 0x85e2a6,
    support: 0x57caee,
    spring: 0x9ad86a,
    coupling: 0xf3a86a,
    selected: 0xff6b35,
    grid: 0x3a4c58,
    gridCenter: 0x5f7a89,
    cubeFace: 0x2b3f4c,
    cubeBorder: 0x7fa8c2,
    cubeText: 0xdcecf5,
    cubeHover: 0x76d5f2,
    cubeSelected: 0x25b8e8,
  }),
  paper: Object.freeze({
    label: "Paper",
    background: 0xeee9df,
    member: 0x5c809d,
    truss: 0x4d8b92,
    cable: 0x7a75ab,
    ineffective: 0x968f83,
    shell: 0x84aecb,
    shellEdge: 0x304956,
    node: 0x61aa73,
    support: 0x3d9cbe,
    spring: 0x6aad3c,
    coupling: 0xd07a38,
    selected: 0xff6b35,
    grid: 0xb3b1a6,
    gridCenter: 0x7d8378,
    cubeFace: 0xf3efe6,
    cubeBorder: 0x4f7288,
    cubeText: 0x27485c,
    cubeHover: 0x76d5f2,
    cubeSelected: 0x25b8e8,
  }),
  white: Object.freeze({
    label: "White",
    background: 0xffffff,
    member: 0x5e8bb1,
    truss: 0x4f93a4,
    cable: 0x7d7ab8,
    ineffective: 0xa4adb4,
    shell: 0x85b2d7,
    shellEdge: 0x2a4359,
    node: 0x61b571,
    support: 0x3dbeee,
    spring: 0x76c043,
    coupling: 0xe08a4a,
    selected: 0xff6b35,
    grid: 0xc3d2da,
    gridCenter: 0x8ba2ae,
    cubeFace: 0xfbfdfe,
    cubeBorder: 0x4a7ea6,
    cubeText: 0x1d4b6d,
    cubeHover: 0x76d5f2,
    cubeSelected: 0x25b8e8,
  }),
});

const APPEARANCE_IDS = Object.freeze(Object.keys(APPEARANCES));

function appearanceDefinition(name) {
  return APPEARANCES[name] || APPEARANCES.cloud;
}

module.exports = { APPEARANCES, APPEARANCE_IDS, appearanceDefinition };
